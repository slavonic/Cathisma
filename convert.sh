...
$FONTFORGE_BIN -lang=ff -c 'Open($1); Generate($2)' sources/Cathisma.sfd sources/Cathisma-Regular.ufo

# Dirty hack to add the openTypeOS2Selection key to the fontinfo.plist file
FONTINFO=`cat sources/Cathisma-Regular.ufo/fontinfo.plist`
echo "${FONTINFO/    <key>openTypeOS2Type<\/key>/    <key>openTypeOS2Selection</key>$'\n'    <array>$'\n'      <integer>7</integer>$'\n'    </array>$'\n'    <key>openTypeOS2Type</key>}" > sources/Cathisma-Regular.ufo/fontinfo.plist
